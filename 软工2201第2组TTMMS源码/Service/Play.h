...
//剧目类型定义 
typedef enum {
	PLAY_TYPE_FILE=1,     //1表示电影
	PLAY_TYPE_OPEAR=2,    //2表示歌剧
	PLAY_TYPE_CONCERT=3   //3表示音乐会
} play_type_t;
 //演出级别类型定义
typedef enum {
	PLAY_RATE_CHILD = 1,     // 1表示儿童可观看
	PLAY_RATE_TEENAGE = 2,   // 2表示青少年可观看
	PLAY_RATE_ADULT = 3       // 3表示成人可观看
} play_rating_t;
//剧目信息定义
typedef struct {
	int id;                  //信息ID号
...
typedef struct play_node {
	play_t data;//实体数据 
	struct play_node *next, *prev;//前向，后向指针 
